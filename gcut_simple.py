...
print(kells.shape)
sym_shape, sym_wcs = enmap.geometry(res=np.deg2rad(1.5/60.), pos=[0,0], shape=(2000,2000), proj="plain")
...
## Filter alms
Xdats = [utils.isotropic_filter(lensed_websky_alm,tcls,lmin,lmax) for lmax in lmaxes] 
gXdats = [utils.isotropic_filter(lensed_websky_alm,tcls,lmin,glmax) for glmax in glmaxes]
...
for est in ests:
    # loop over all (glmax,lmax) recons
    for j in range(len(recons)):
        recon, Als = recons[j], Als_sym[j]

        pl = io.Plotter('CL')
        pl._ax.set_title("recon vs input for \'%s\' est, (glmax=%d, lmax=%d)" % (est, glmaxes[j], lmaxes[j]))
        pl2 = io.Plotter('rCL',xyscale='loglin')
        pl2._ax.set_title("Delta C_l^(k-hat,k) / C_l^(kk) %% error for \'%s\' est (glmax=%d, lmax=%d)" % (est, glmaxes[j], lmaxes[j]))
