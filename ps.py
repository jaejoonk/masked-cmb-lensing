...
MAP2_FILENAME = "inpainted_map_ivar_SNR_5.fits"
...
lensed_cls = hp.alm2cl(lensed_alm[0].astype(np.cdouble),
                       lensed_alm[0].astype(np.cdouble),
                       lmax=MLMAX)
...
pl_tt.add(*bin((inpainted_cls - lensed_cls) / lensed_cls), marker='o', label="inp. vs non-inp.")
...
pl_tt.hline(y=0)
pl_tt.done(f"ps_cltt_with_null_plus_ivar_{ESTS[0]}_3000.png")
...
Xdats = futils.isotropic_filter(lensed_alm, tcls, lmin=LMIN, lmax=LMAX)
...
for est in ESTS:
    ...
    pl2.add(*bin2((ixinp_cls-icls)/icls),marker='o',label="inpaint x input Clkk")
    pl2.add(*bin2((ixinp2_cls-icls)/icls),marker='o',label="inpaint (ivar) x input Clkk")
    pl2.add(*bin2((rxi_cls-icls)/icls),marker='o',label="recon x input Clkk")
    pl2.add(*bin2((ixfinp_cls-icls)/icls),marker='o',label="null inpaint x input Clkk")
    #pl2.add(*bin2((inpainted_cls-lensed_cls)/lensed_cls), marker='o', label="inp. ClTT vs non-inp. ClTT")
    ...
    pl.done(f'ps_all_ivar_{est}_3000.png')
    pl2.done(f'ps_cross_vs_auto_diff_ivar_{est}_3000.png')
